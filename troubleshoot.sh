...
if [ -d "frontend/node_modules" ]; then
    ...
    echo "⚠️  Frontend dependencies missing - run ./frontend-setup.sh"
fi
...
if [ -z "$OPENAI_API_KEY" ]; then
    ...
    echo "✅ OPENAI_API_KEY configured (length: ${#OPENAI_API_KEY} chars)"
fi
...
if command -v lsof &> /dev/null; then
    ...
    echo "⚠️  Cannot check port usage (lsof/netstat not available)"
fi
