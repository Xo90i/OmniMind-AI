...
import { Upload, File, CheckCircle, AlertCircle, Loader2, Database, ShieldCheck, Cpu } from "lucide-react";
...
export default function DocumentsPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/intel/upload`, {
        method: "POST",
        body: formData,
      });
    ...
    <AppLayout>
      <div className="max-w-6xl mx-auto p-6">
        <header className="mb-10">
          ...
          <p className="text-zinc-400 text-lg">Upload knowledge assets to expand the platform's intelligence with RAG and pgvector.</p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-zinc-900/50 border-2 border-dashed border-zinc-800 rounded-3xl p-12 flex flex-col items-center justify-center text-center transition-all hover:border-blue-500/50 group">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Select relevant files</h3>
              <p className="text-zinc-500 mb-8 max-w-sm">Support for PDF, DOCX, TXT, and CSV. Files are chunked and embedded automatically.</p>
            ...
            {status && (
              <div className={`p-6 rounded-2xl flex items-center gap-4 animate-in slide-in-from-top-4 duration-300 ${status.type === "success" ? "bg-emerald-950/30 border border-emerald-800 text-emerald-400" : "bg-red-950/30 border border-red-800 text-red-400"}`}>
          ...
          <div className="space-y-6">
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
              ...
              <ul className="space-y-4 text-sm">
                <li className="flex justify-between border-b border-zinc-800 pb-2">
                  ...
                  <span className="text-zinc-300 font-mono">pgvector-RS256</span>
                </li>
                <li className="flex justify-between border-b border-zinc-800 pb-2">
            ...
            <div className="bg-gradient-to-br from-blue-600 to-purple-700 rounded-2xl p-8 text-white shadow-2xl">
              ...
              <p className="text-blue-100/80 leading-relaxed">Your knowledge documents are processed locally and securely stored as vector embeddings in our PostgreSQL vault.</p>
            </div>
          </div>
        </div>
      </div>
