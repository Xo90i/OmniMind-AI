...
export default function Features({ onAction }: FeaturesProps) {
  return (
    <section id="features" className="py-32 sm:py-48 bg-[var(--bg-main)] text-[var(--text-primary)] relative">
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10">
        <div className="mx-auto max-w-4xl text-center">
          ...
          >
            A new standard in autonomous collaboration. Discover the modules that power the OmniMind ecosystem.
          </motion.p>
        </div>
            ...
            {features.map((feature, index) => (
              ...
              >
                <dt className="flex items-center gap-x-4 text-lg font-bold text-[var(--text-primary)] mb-6">
                  <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl ${feature.color} border border-[var(--border-primary)] group-hover:border-blue-600/50 transition-all duration-500 shadow-lg`}>
                    <feature.icon className="h-7 w-7" aria-hidden="true" />
                  </div>
                  {feature.name}
                </dt>
                <dd className="flex flex-auto flex-col text-sm leading-7 text-[var(--text-secondary)] font-medium">
                  <p className="flex-auto group-hover:text-[var(--text-primary)]/60 transition-colors duration-500">{feature.description}</p>
