...
export default function Pricing({ onAction }: PricingProps) {
  return (
    <section id="pricing" className="py-32 sm:py-48 bg-[var(--bg-main)] text-[var(--text-primary)] relative">
          ...
          {tiers.map((tier, index) => (
            <motion.div
              ...
              className={`card p-10 flex flex-col relative ${
                tier.popular ? 'border-blue-600/50 shadow-[0_20px_50px_rgba(59,130,246,0.1)] scale-105 z-10' : ''
              }`}
            >
              {tier.popular && (
              ...
              <div className="flex items-center justify-between mb-8">
                  <div className={`w-14 h-14 rounded-2xl bg-[var(--glass-bg)] flex items-center justify-center border border-[var(--border-primary)] ${tier.popular ? 'border-blue-600/20 bg-blue-600/10' : ''}`}>
                    <tier.icon className={`w-7 h-7 ${tier.popular ? 'text-blue-600' : 'text-[var(--text-secondary)]'}`} />
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-bold text-[var(--text-secondary)] tracking-wider uppercase mb-1">{tier.name}</div>
                    <div className="text-3xl font-bold text-[var(--text-primary)] tracking-tight">
                      {tier.price !== 'Custom' && '$'}{tier.price}
                      {tier.price !== 'Custom' && <span className="text-xs text-[var(--text-secondary)]/50 font-medium">/mo</span>}
                    </div>
                  </div>
              </div>

              <p className="text-[var(--text-secondary)] text-sm font-medium mb-10">{tier.description}</p>

              <ul className="space-y-4 mb-12 flex-1">
                {tier.features.map(feature => (
                  <li key={feature} className="flex items-center text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-widest">
